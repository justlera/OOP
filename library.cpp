...
void Library::Del(string name, string author)
{
    if (Head == NULL) { cout << "\nСписок пуст\n\n"; }
    else{
        Node* temp = Head;
        ...
        /*while (temp->Next!=temphead && temp->Next->pBook->getname() != name && temp->Next->pBook->getauthor() != author){
            temp = temp->Next;
            flag = 1;
        }*/
        while (temp->Next != temphead && flag != 1){
            if (temp->Next->pBook->getname() == name && temp->Next->pBook->getauthor() == author){
                flag = 1;
            }
            else{
                temp = temp->Next;
